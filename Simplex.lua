...
--// Constants
--// Note that these are used due to number types complications.
local INT_POSITIVE_LIMIT =  16777215
local INT_NEGATIVE_LIMIT = -16777216
...
local Base3D = function(xr, yr, zr)
	...
	local LatticePoint = Lattice3D_Lookup[bit_or(xri >= 0.5 and 1 or 0, yri >= 0.5 and 2 or 0, zri >= 0.5 and 4 or 0)] --// Left shifts. Multiplying seems to be ~30 % faster than using shifts.
...
function Noise.Seed(Seed) --// undefined behavior where x does not fall within the [-2^23, 23^23-1] range. Decimal places already generate different maps.
	...
	for i = PMASK, 0, -1 do
		Seed = Seed * 11576296 + 2624246

		--// Simulate integer overflow with voodoo magic.
		if Seed >= INT_POSITIVE_LIMIT then
			Seed = (Seed % INT_POSITIVE_LIMIT) - INT_POSITIVE_LIMIT
		elseif Seed <= INT_NEGATIVE_LIMIT then
			Seed = (Seed % INT_POSITIVE_LIMIT) + INT_POSITIVE_LIMIT
		end
